...
int main(int argd, char *argv[]) {
    ...
    printf("PING %s(%s): %d data bytes\n", dest_ip, dest_ip, bytes_sent);
    /* printf("Successfully sent one packet : ICMP HEADER : %d bytes, data length : %d , icmp header : %d \n", bytes_sent,
            datalen, ICMP_HDRLEN);*/
    ...
    while ((bytes_received = recvfrom(sock, packet, sizeof(packet), 0, (struct sockaddr *) &dest_in, &len))) {
        if (bytes_received > 0) {
            ...
            /*   printf("Successfully received one packet with %zd bytes : data length : %d , icmp header : %d , ip header : %d \n",
...
//    printf("\nRTT: %f milliseconds (%ld microseconds)\n", milliseconds, microseconds);
    ...
    printf("%zd bytes form %s: icmp_seq=%d tt1=%d time=%f ms\n", bytes_received, dest_ip,  icmphdr.icmp_seq, 117,
           milliseconds);
